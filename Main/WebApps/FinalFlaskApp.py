...
def printConnection(x,SA):
    ...
    return "{} {} to {} {}:{} {}:{} {}".format(x.train_no,SA[x.dept_stn],SA[x.arr_stn],hr1,min1,hr2,min2,x.label)
...
def Itinerary(IC,Departure_Station,Arrival_Station,SA):
    ...
    if IC[Arrival_Station] == None :
        ...
        while prev_connection != None:
            ...
            if no_of_iters == 0:
                lis.append("Deadlock")
                lis.append(Departure_Station+" "+Arrival_Station)
                return jsonify(results=lis)
            #print 'Out here'

        for x in Route:
            lis.append(printConnection(x,SA)+"\n")
...
def my_form_post():
  ...
  t.append("----:"+ SA[src]+ "to"+ SA[dst]+ "(with tCSA):----")
  print "----:", SA[src], "to", SA[dst], "(with tCSA):----"
  t.append(tCSA(src,dst,randint(1,23)*60*60,DCTable,SA,TT,OMTable,CA))
